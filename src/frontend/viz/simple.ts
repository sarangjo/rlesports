...
import { concat, forEach, reduce } from "lodash";
...
interface SimpleViz extends RLVisualization {
  process: (tournaments: Tournament[]) => { nodes: TournamentNode[]; links: TournamentLink[] };
}

const simpleViz: SimpleViz = {
  main: async (chart: Chart) => {
    ...
    const data = simpleViz.process(tournaments);
